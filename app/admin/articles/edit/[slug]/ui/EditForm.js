...
import Spinner from '@/app/admin/components/Utility/Spinner'
import putEditArticle from '../../../(actions)/EditAction'
...
export default function EditForm({slug}) {
  ...
  const [pageLoading, sePageLoading] = useState(false)
  ...
  useEffect(() => {
        ...
        try {
            ...
            const matchedCat = categories.find(cat => cat.value === data.category_id)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await putEditArticle(articleData,slug);
  ...
  return (
    <form className="formContainer" onSubmit={handleSubmit}>
        ...
        <div className="side-content">
          <div className="form-control">
            ...
            {fileLoading ? (
              ...
              <>
                <img style={{width:'100%',height:'auto'}} src={tbuFile} alt="Uploaded preview" />
      ...
      <div className="form-control">
        ...
        <input defaultValue={article.title} type="text" name="title" id="title" required />
      </div>

      <div className="form-control">
        <label htmlFor="content">Article Content:</label>
        <MyEditor content={article.content}  setEditorOutput={setEditorOutput} />
      </div>

      <button type="submit" disabled={loading}>
        {loading ? (
          <>
            Edit Article <Spinner />
          </>
        ) : (
          'Edit Article'
        )}
